/*	$Id: test-html-noscope.c,v 1.1 2020/03/27 22:17:10 kristaps Exp $ */
/*
...
child(void)
{
	...
	if (khtml_closeelem(&req, 0) != KCGI_OK ||
	    khtml_closeelem(&req, 100) != KCGI_OK) {
		warnx("khtml_closeelem");
		goto out;
	}

	if (khtml_closeto(&req, 0) != KCGI_OK ||
	    khtml_closeto(&req, 100) != KCGI_OK) {
		warnx("khtml_closeto");
		goto out;
	}
	...
	if (khtml_closeto(&req, pos) != KCGI_OK) {
		warnx("khtml_closeto");
		goto out;
	} else if (pos != khtml_elemat(&req)) {
		warnx("should not have closed scopes");
		goto out;
	}

	if (khtml_puts(&req, "foo") != KCGI_OK) {
		warnx("khtml_puts");
		goto out;
	} else if (pos != khtml_elemat(&req)) {
		warnx("should not have closed scopes");
